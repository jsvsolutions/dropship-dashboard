...
export default function KanbanBoard() {
  ...
  const [form, setForm] = useState({ produto: "", plataforma: "facebook", gasto: "", faturamento: "", cpa: "", stage: "contingencia" });
  ...
  async function loadCards() {
    const { data } = await supabase.from("produtos").select("*").order("created_at", { ascending: true });
  ...
  async function addCard() {
    ...
    const { data } = await supabase.from("produtos").insert([form]).select().single();
    if (data) setCards((prev) => [...prev, data]);
    setShowModal(false);
    setForm({ produto: "", plataforma: "facebook", gasto: "", faturamento: "", cpa: "", stage: "contingencia" });
  }

  async function moveCard(cardId: string, targetStage: string) {
    await supabase.from("produtos").update({ stage: targetStage }).eq("id", cardId);
            ...
            return (
              <div key={stage.id} onDragOver={(e) => e.preventDefault()} onDrop={() => dragging && moveCard(dragging, stage.id)}
                ...
                <div style={{ padding: "10px 12px", borderBottom: "1px solid var(--border)", display: "flex", alignItems: "center", gap: "7px" }}>
                  ...
                  <span style={{ marginLeft: "auto", fontSize: "11px", color: "var(--text-muted)", backgroundColor: "var(--bg-hover)", padding: "1px 7px", borderRadius: "10px" }}>{stageCards.length}</span>
                </div>
                  ...
                  {stageCards.map((card) => (
                    <div key={card.id} draggable onDragStart={() => setDragging(card.id)} onDragEnd={() => setDragging(null)}
                      ...
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "8px" }}>
                        ...
                        <button onClick={() => removeCard(card.id)} style={{ background: "none", border: "none", cursor: "pointer", color: "var(--text-muted)", padding: 0 }}><X size={12} /></button>
                      </div>
                      <div style={{ display: "inline-block", padding: "2px 7px", borderRadius: "4px", fontSize: "10px", fontWeight: 600, backgroundColor: (PLATAFORMA_COLORS[card.plataforma] || "#888") + "22", color: PLATAFORMA_COLORS[card.plataforma] || "#888", marginBottom: "8px", textTransform: "capitalize" }}>
                        {card.plataforma}
                      </div>
